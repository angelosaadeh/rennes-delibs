"""Download the local models into the paths from config.py (skips if present).

    python download_models.py               # e5 (+ LLaMA if LLM_BACKEND=local)
    python download_models.py --embed-only  # only e5 — enough to update the index
...
def main():
    ...
    parser.add_argument(
        "--embed-only",
        action="store_true",
        help="Download only the e5 embedding model (skip LLaMA). Enough to run update.py.",
    )
    args = parser.parse_args()

    # e5 is always needed: it embeds chunks (and questions at query time).
    print(f"e5 embedding model -> {E5_MODEL_PATH}")
    for f in E5_FILES:
        download(f"{E5_BASE}/{f}", os.path.join(E5_MODEL_PATH, *f.split("/")))

    # The big LLaMA GGUF is only needed to generate answers locally — not to
    # update the index, and not when Groq runs generation hosted.
    if args.embed_only:
        print("--embed-only: skipping the LLaMA download.")
